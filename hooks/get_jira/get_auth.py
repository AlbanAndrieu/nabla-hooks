    ...
    try:
        if not user:
            if verbose:
                print(
                    colored(
                        "User is empty, using JIRA_USER environement variable.",
                        "yellow",
                    ),
                )
        ...
        print(
            colored(
                "Please set the environment variable JIRA_USER or GIT_USERNAME",
                "red",
            ),
        )
        sys.exit(3)
    ...
    try:
        if not password:
            if verbose:
                print(
                    colored(
                        "Password is empty, using JIRA_PASSWORD environement variable",
                        "yellow",
                    ),
                )
        ...
        print(
            colored(
                "Please set the environment variable JIRA_PASSWORD or GIT_PASSWORD",
                "red",
            ),
        )
        sys.exit(3)
